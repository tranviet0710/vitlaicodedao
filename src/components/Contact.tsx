    ...
    <section id="contact" className="py-20 md:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          >
            <div>
              <h3 className="text-2xl font-bold mb-6 font-heading">{t('contact.infoTitle')}</h3>
                  ...
                  return (
                    ...
                    >
                      <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center transition-colors group-hover:bg-primary/20">
                        <Icon className="w-6 h-6 text-primary" style={{ filter: `drop-shadow(0 0 5px hsla(var(--primary), 0.7))`}} />
                      </div>
                      <div>
            ...
            <div>
              <h3 className="text-2xl font-bold mb-6 font-heading">{t('contact.socialTitle')}</h3>
                ...
                {socialLinks.map((social, index) => {
                  ...
                  return (
                    <a
                      ...
                      title={social.name}
                    >
                      <Icon className="w-6 h-6 text-primary/80 group-hover:text-primary transition-colors"  style={{ filter: `drop-shadow(0 0 5px hsla(var(--primary), 0.7))`}}/>
                    </a>
                  );
                })}
              </div>
          ...
          >
             ...
             <div className="relative">
              <h3 className="text-2xl font-bold mb-6 font-heading">{t('contact.formTitle')}</h3>
              
              {isSuccess && (
                <div className="mb-6 p-4 bg-primary/10 border border-primary/20 rounded-lg flex items-center gap-3">
                  <CheckCircle2 className="h-5 w-5 text-primary" />
                  <p className="text-sm text-primary">{t('contact.successMessage')}</p>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <Input
                    ...
                    className={`bg-background/70 focus:bg-background transition-all ${errors.name ? 'border-destructive' : 'border-primary/30 focus:border-primary'}`}
                ...
                <div>
                  <Input
                    ...
                    className={`bg-background/70 focus:bg-background transition-all ${errors.email ? 'border-destructive' : 'border-primary/30 focus:border-primary'}`}
                ...
                <div>
                  <Textarea
                    ...
                    className={`bg-background/70 focus:bg-background resize-none transition-all ${errors.message ? 'border-destructive' : 'border-primary/30 focus:border-primary'}`}
