...
const generateBlogSitemap = async () => {
  try {
    ...
    if (error) {
      console.warn('⚠️  Could not fetch blogs from database, creating empty blog sitemap');
    }
    ...
    writeFileSync(
      join(__dirname, '../public/sitemap-blogs.xml'),
      sitemap
    );
...
const generateProjectSitemap = async () => {
  try {
    ...
    if (error) {
      console.warn('⚠️  Could not fetch projects from database, creating empty project sitemap');
    }
    ...
    writeFileSync(
      join(__dirname, '../public/sitemap-projects.xml'),
      sitemap
    );
    
    console.log(`✅ Project sitemap generated with ${projects?.length || 0} projects!`);
  } catch (error) {
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  writeFileSync(
    join(__dirname, '../public/sitemap-index.xml'),
    sitemapIndex
  );
